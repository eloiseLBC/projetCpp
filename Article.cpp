...
Article::Article(const string& title, const string& author, bool borrowed,
                 int publicationYear, int numberPages, const string& genre,
                 const string& summary, const string& editor, int numberArticles,
                 const string& name)
    : Review(title, author, borrowed, publicationYear, numberPages, genre, summary, editor, numberArticles) {
    this->name = name;
}
